...
TEST(Validation, TrickyCases)
{
    ...
    EXPECT_TRUE(matcher.IsValid("http://[2001:db8::1]:8080/?query=complex&value=1")) 
        << "ipv6 and query";

    EXPECT_TRUE(matcher.IsValid("http://host?")) 
        << "empty query";
}
...
TEST(Validation, Http)
{
    ...
    EXPECT_FALSE(matcher.IsValid("http://:8080/path"))
        << "authority: no host";
    EXPECT_FALSE(matcher.IsValid("http:///path"))
        << "authority: no host";
    EXPECT_FALSE(matcher.IsValid("http://"))
